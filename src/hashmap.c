...
static int hashmap_rehash(struct hashmap *map, size_t new_size)
{
    ...
    new_table = (struct hashmap_entry *)nat_calloc_root(new_size, sizeof(struct hashmap_entry));
    if (!new_table) {
        return -ENOMEM;
    }
    ...
    size_t initial_size)
{
    ...
    map->num_entries = 0;
    map->table = (struct hashmap_entry *)nat_calloc_root(initial_size, sizeof(struct hashmap_entry));
    if (!map->table) {
        return -ENOMEM;
    }
    map->hash = hash_func ?
...
void hashmap_reset(struct hashmap *map)
{
    ...
    new_table = (struct hashmap_entry *)nat_realloc_root(map->table,
